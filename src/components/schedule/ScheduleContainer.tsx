...
export function ScheduleContainer() {
  ...
  const { messages, isLoading, create, toggle, remove } = useScheduledMessages();
    ...
    <div className="min-h-screen bg-gray-950">
      ...
      <header className="flex items-center justify-between px-4 py-3 border-b border-gray-800 bg-gray-950">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-lg font-bold text-gray-100">Scheduled Messages</h1>
          {messages.length > 0 && (
            <span className="text-xs font-semibold text-gray-500 bg-gray-800 px-2 py-0.5 rounded-full">
              {messages.filter((m) => m.enabled).length} active
            </span>
          )}
        </div>
