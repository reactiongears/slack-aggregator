#!/bin/bash
# ============================================================
# Slack Aggregator — macOS Installer
# Double-click this file to install.
#
# Creates a temporary native .app installer and opens it.
# All user interaction is through macOS dialogs — no terminal.
# ============================================================
...
TMPDIR=$(mktemp -d)
TMPAPP="$TMPDIR/Slack Aggregator Installer.app"

# Build installer .app with project path embedded
cat > "$TMPDIR/installer.applescript" << 'ENDSCRIPT'
set projectDir to "PLACEHOLDER_DIR"
...
try
    ...
    if (majorVer as integer) ≥ 18 then
        ...
        display dialog "Node.js " & nodeVer & " was found but version 18+ is required." & return & return & "Please upgrade Node.js and try again." buttons {"OK"} default button 1 with title "Slack Aggregator" with icon stop
        return
    end if
on error
    -- Node not found
...
if not nodeFound then
    ...
    if brewFound then
        try
            display dialog "Node.js is required but not installed." & return & return & "Homebrew was detected. Install Node.js via Homebrew?" buttons {"Cancel", "Install Node.js"} default button "Install Node.js" with title "Slack Aggregator" with icon note
        on error
            return
        end try
        ...
        on error errMsg
            display dialog "Failed to install Node.js:" & return & return & errMsg buttons {"OK"} default button 1 with title "Slack Aggregator" with icon stop
            return
        end try
    else
        try
        ...
        on error errMsg
            display dialog "Failed to install Homebrew:" & return & return & errMsg buttons {"OK"} default button 1 with title "Slack Aggregator" with icon stop
            return
        end try
    ...
    try
        set nodeVer to do shell script envSetup & "node -v"
    on error
        display dialog "Node.js installation may have succeeded but it can't be found." & return & return & "Please open Terminal and run:" & return & "  bash install.sh" buttons {"OK"} default button 1 with title "Slack Aggregator" with icon stop
        return
    end try
end if
...
on error errMsg
    display dialog "Failed to install dependencies:" & return & return & errMsg buttons {"OK"} default button 1 with title "Slack Aggregator" with icon stop
    return
end try
...
on error errMsg
    display dialog "Build failed:" & return & return & errMsg buttons {"OK"} default button 1 with title "Slack Aggregator" with icon stop
    return
end try
...
on error errMsg
    display dialog "Failed to create app:" & return & return & errMsg buttons {"OK"} default button 1 with title "Slack Aggregator" with icon stop
    return
end try
...
# Inject the actual project directory
sed -i '' "s|PLACEHOLDER_DIR|${DIR}|" "$TMPDIR/installer.applescript"

# Compile and launch
osacompile -o "$TMPAPP" "$TMPDIR/installer.applescript"
open "$TMPAPP"
